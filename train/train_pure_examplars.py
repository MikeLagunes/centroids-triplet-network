...
from double_resnet_softmax import *
from loss import *
from utils import *
...
def train(args):
    ...
    # Setup Dataloader
    data_loader = get_loader('double_resnet_' + args.dataset +'_softmax')
    ...
    # Setup Model
    model = double_resnet50_softmax(pretrained=True,  num_classes=n_classes, embedding_size=args.embedding_size)
    model.cuda()
    model.train()

    optimizer = optim.SGD(model.parameters(), lr=args.l_rate, momentum=0.9, weight_decay=args.wd)#, weight_decay=1e-5

    loss_fn = PureExemplarSoftmaxLoss(lambda_factor=args.lambda_factor, margin=args.margin)
    ...
    for epoch in range(args.n_epoch):
        ...
        for i, (images, images_neg, path_img, labels_anchor, labels_neg) in enumerate(trainloader):
            ...
            optimizer.zero_grad()
            embed_anch, embed_neg, predictions  = model(images, images_neg)

            #exemplars_torch

            loss, loss_softmax, loss_nby = loss_fn(embed_anch, embed_neg, predictions, labels_anchor, labels_neg, exemplars_torch)
            ...
            if global_step % args.logs_freq == 0:
                ...
                log_loss(epoch, global_step, loss_sum=loss.item(), loss_softmax=loss_softmax.item(), loss_nby=loss_nby.item() ) 
        ...
        if epoch % 4  == 0:
            ...
            for i, (images, images_neg, path_img, labels_anchor, labels_neg) in enumerate(trainloader):
                ...
                embed_anch, _, _  = model(images, images)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', nargs='?', type=str, default='double_cnn_softmax',
