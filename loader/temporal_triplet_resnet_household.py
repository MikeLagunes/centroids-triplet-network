...
def get_different_object(filename, instances=''):
    ...
    if next_obj_comp == obj_curr_comp:
...
def get_nby_views(filename, closeness):
    ...
    elif index_current == total_candidates - 1: 
        index_current -= 1
        
    filename = os.path.join(os.path.split(filename)[0],candidates[index_current])

    if (index_current + closeness) < total_candidates:
        img_path_ahead = candidates [index_current + closeness]
        img_path_before =  candidates [index_current - closeness]

    else:
        view_close = candidates [index_current - closeness]

    img_path_ahead = os.path.join(os.path.split(filename)[0],img_path_ahead)
    img_path_before = os.path.join(os.path.split(filename)[0],img_path_before)
    
    return filename, img_path_ahead, img_path_before



class temporal_triplet_resnet_household(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_path, img_path_similar, img_path_different = get_nby_views(img_path, closeness=1)
        ...
        return img_pos, img, img_neg, img_path, lbl, lbl_pos, lbl_neg
...
if __name__ == '__main__':
    ...
    dst = temporal_triplet_resnet_household(local_path, is_transform=True, augmentations=None,split="train", 
    ...
    for i, data in enumerate(trainloader):
        ...
        plt.show()#plt.pause(0.5)#plt.show()
