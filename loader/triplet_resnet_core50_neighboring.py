...
def get_nby_view(filename, closeness=3):
    ...
    if (index_current + closeness) < total_candidates:
        view_close = candidates [index_current + 1]
...
class triplet_resnet_core50_neighboring(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split[0:5] == "train" :
            ...
            img_path_close, img_path_far = get_nby_view(img_path, self.closeness)
        ...
        if self.is_transform:
            ...
            img_close, img_far, _ = self.transform(img_close, img_far, img_far )


            lbl, lbl_pos, lbl_neg = self.transform_lbl(lbl, lbl_pos, lbl_neg)

        return img, img_pos, img_neg, img_close, img_far, img_path, lbl, lbl_pos, lbl_neg
...
if __name__ == '__main__':
    ...
    t_loader = triplet_resnet_core50_neighboring(root=local_path, is_transform=True, 
        split='train',
        img_size=(224, 224), 
        augmentations=None, 
        instances=instances)
    ...
    for i, data in enumerate(trainloader):
        imgs, imgs_pos, imgs_neg, imgs_pos_nby, imgs_neg_nby, img_path, lbl, lbl_pos, lbl_neg = data
        ...
        for j in range(bs):      
            ...
            axarr[j][4].imshow(imgs_neg_nby[j])
