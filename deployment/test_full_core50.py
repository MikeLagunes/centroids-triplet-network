...
def test(args):
    ...
    classifier = pickle.load(open("/home/alexa/Miguel/ste-network/deployment/core50_nc.sav", 'rb'))
   ...
   # print(correct, total)
        ...
        ax.set_title('Core50', fontsize=20, color="blue")

        ax2.set_xticks([])
        ax2.set_yticks([])
        ax2.set_xlabel('    Ground truth: \n   ' + str(labels_core[int(labels)]) ,fontsize=13)


        img_root = glob.glob("/media/alexa/DATA/Datasets/core50/train/obj_{:02d}_scene_01/*".format(int(labels)+1))
        img_root.sort()
        ...
        ax3.set_xlabel('    Object predicted: \n ' + labels_core[int(result)] + "\n   Class Probability:\n %.2f" % probs[0][best_n][0][0], fontsize=13, color=color)
        
        img_root = glob.glob("/media/alexa/DATA/Datasets/core50/train/obj_{:02d}_scene_01/*".format(int(result[0])+1))
        img_root.sort()
        ...
        plt.savefig("/media/alexa/DATA/Miguel/checkpoints_alexa/icra2020/core50/%d.pdf"%i)
        ax.clear()
        ax2.clear()
        ax3.clear()
