...
def test(args):
    ...
    classifier = pickle.load(open("/home/alexa/Miguel/ste-network/deployment/toybox_nc.sav", 'rb'))
    ...
    #print(correct, total)
       # ax.set_xlabel('Prediction: ' + labels_tejani[int(result)] + "\n Class Probability: %.4f" % probs[0][best_n][0][0], fontsize=16, color=color)
        ...
        ax.set_title('ToyBox', fontsize=20, color="blue")

        ax2.set_xticks([])
        ax2.set_yticks([])
        ax2.set_xlabel('    Ground truth: \n   ' + str(labels_toybox[int(labels)]) ,fontsize=13)


        img_root = glob.glob("/media/alexa/DATA/Datasets/toybox/train/{}/*".format(labels_toybox[int(labels)]))
        img_root.sort()
        ...
        ax3.set_xlabel('    Object predicted: \n ' + labels_toybox[int(result)] + "\n   Class Probability:\n %.2f" % probs[0][best_n][0][0], fontsize=13, color=color)
        
        img_root = glob.glob("/media/alexa/DATA/Datasets/toybox/train/{}/*".format(labels_toybox[int(result[0])]))
        img_root.sort()
        ...
        plt.savefig("/media/alexa/DATA/Miguel/checkpoints_alexa/icra2020/toybox/%d.pdf"%i)
        ax.clear()
        ax2.clear()
        ax3.clear()
