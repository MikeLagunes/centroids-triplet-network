...
def test(args):
    ...
    classifier = pickle.load(open("/home/alexa/Miguel/ste-network/deployment/household_nc.sav", 'rb'))
    ...
    #print(correct, total)
       # ax.set_xlabel('Prediction: ' + labels_tejani[int(result)] + "\n Class Probability: %.4f" % probs[0][best_n][0][0], fontsize=16, color=color)
        ...
        ax.set_title('In-situ household', fontsize=20, color="blue")

        ax2.set_xticks([])
        ax2.set_yticks([])
        ax2.set_xlabel('    Ground truth: \n   ' + str(labels_household[int(labels)]) ,fontsize=13)


        img_root = glob.glob("/home/alexa/Miguel/dataset_temp/insitu-household/train/{}-1/*".format(labels_household[int(labels)]))
        img_root.sort()
        ...
        ax3.set_xlabel('    Object predicted: \n ' + labels_household[int(result)] + "\n   Class Probability:\n %.2f" % probs[0][best_n][0][0], fontsize=13, color=color)
        
        img_root = glob.glob("/home/alexa/Miguel/dataset_temp/insitu-household/train/{}-1/*".format(labels_household[int(result[0])]))
        img_root.sort()
        ...
        plt.savefig("/media/alexa/DATA/Miguel/checkpoints_alexa/icra2020/video_house2/%d.png"%i)
        ax.clear()
        ax2.clear()
        ax3.clear()
